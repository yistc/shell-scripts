...
if [[ "$ARCH" == "x86_64" ]]; then
    curl -L https://dl.nssurge.com/snell/snell-server-v4.1.1-linux-amd64.zip -o snell.zip
    unzip snell.zip
    chmod +x snell-server && mv snell-server /usr/local/bin/snell
    rm snell.zip
elif [[ "$ARCH" == "arm64" || "$ARCH" == "aarch64" ]]; then
    curl -L https://dl.nssurge.com/snell/snell-server-v4.1.1-linux-aarch64.zip -o snell.zip
    unzip snell.zip
    chmod +x snell-server && mv snell-server /usr/local/bin/snell
    rm snell.zip
else
